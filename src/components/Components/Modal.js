...
export default function Modal(props) {
	...
	useEffect(() => {
		// Кодът, който искате да се изпълни при промяна в isOpen
		console.log(isOpen)
	}, [isOpen])
		...
		<>
			...
			{isOpen && (
				<div
					id="defaultModal"
					aria-hidden={!isOpen}
					className="fixed inset-0 z-[52] flex justify-center overflow-auto"
				>
					<div className="w-full max-w-2xl pt-8">
						<div className="relative bg-white rounded-lg shadow pb-16 mb-8">
							...
							<button
								type="button"
								className="absolute bottom-4 left-32 text-gray-500 bg-white hover:bg-gray-100 rounded-lg border border-gray-200 text-sm font-semibold px-5 py-2.5 hover:text-gray-900 focus:z-10 transition active:scale-95"
								onClick={toggleModal}
							>
								Затвори
							</button>
			...
			{isOpen && (
				<div
					className="bg-gray-900 bg-opacity-50 fixed inset-0 z-[51]"
					onClick={toggleModal}
				/>
			)}
		</>
