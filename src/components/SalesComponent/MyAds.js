...
import {
    Button, Flex, Box, Spinner, Image, Heading, Text, Avatar,
...
export default function MyAds() {
    ...
    const fetchMyAds = async (page) => {
        try {
            ...
            const response = await fetch(`https://prodavalnik-api.devlabs-projects.info/ads?current`, {
                method: "GET",
                headers: {
            ...
            setAds(
                data.ads.filter(
                    (ad) => (!ad.bought && calculateExpiration(ad.createdAt, ad.expiration, true) > 0)
                )
            )
    ...
    const deleteAdvertisement = async (id) => {
        try {
            const response = await fetch(`https://prodavalnik-api.devlabs-projects.info/ads/${id}`, {
                method: "DELETE",
                user: prodavalnikAuth,
            })
    ...
    return (
        <Box w="-moz-fit-content" marginInline="auto" position="relative" zIndex="1" mb={5}  >
                ...
                {ads.map((ad, index) => (
                    <Flex sx={container} key={index}>
                        <Box>
                            ...
                            <div sx={text && avatar}>
                                Публикувано от: <Avatar name={ad.author.name} verticalAlign="middle" />{ad.author.name}
                            </div>
                            ...
                            <Flex>
                                <Text sx={text} marginInline="auto">
                                    ...
                                    <Button onClick={() => deleteAdvertisement(ad.id)} sx={button}>Изтрий ме</Button>
                                </Text>
                            </Flex>
                        </Box>
